...
- **UIサポート**: 検索結果に含まれる外部サイトへのリンクを自動的に別タブで開く機能や、日付フォーマットを整えるUIユーティリティが含まれています。
...
Pagefind Hubは、検索結果の利便性を高めるための軽量なUIユーティリティを提供しています。

```javascript
import { observePagefind, formatDate, openExternal } from "@mash43/pagefind-hub/ui";

// Pagefind UI の初期化後に呼び出します。
// デフォルトでは formatDate と openExternal の両方が適用されます。
observePagefind();
...
- **`formatDate`**: Pagefindのデフォルトの日付（ISO形式）を、現地のロケール形式（`YYYY-MM-DD`）に変換します。
- **`openExternal`**: 検索結果内の外部リンクに `target="_blank"` と `rel="noopener noreferrer"` を自動的に付与します。
- **`observePagefind`**: Pagefindの結果コンテナを監視し、新しい結果が表示されるたびに指定されたアクションを自動実行します。
