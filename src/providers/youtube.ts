...
const DEFAULT_YOUTUBE_ICON = "https://cdn.jsdelivr.net/gh/gilbarbara/logos/logos/youtube-icon.svg";
...
export function youtube(options: YouTubeProviderOptions): Provider {
  const { channelId, apiKey, limit = 50, image, useThumbnails = true } = options;
  ...
  return {
    ...
    async fetchRecords(): Promise<ProviderRecord[]> {
      ...
      for (const item of data.items) {
        const { title, description, publishedAt, resourceId, thumbnails } = item.snippet;
        ...
        records.push({
          ...
          image: (useThumbnails && videoThumbnail) ? videoThumbnail : effectiveImage,
          meta: {
            title: title,
            date: publishedAt,
            platform: "YouTube",
